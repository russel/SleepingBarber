...
class Main {
  static Void runSimulation(Int numberOfCustomers , Int numberOfWatingSeats ,
    ...
    barber := concurrent::Actor(pool)|Customer customer -> SuccessfulCustomer|{
      ...
      concurrent::Actor.sleep(hairTrimTime())
    ...
    customerFutures := (1 .. numberOfCustomers).map|id|{
      concurrent::Actor.sleep(nextCustomerWaitTime())
  ...
  static Void main() {
    runSimulation(20 , 4 ,
                    |-> sys::Duration|{return sys::Duration(Int.random(0 .. 6000) + 1000)},
                    |-> sys::Duration|{return sys::Duration(Int.random(0 .. 2000) + 1000)})
  }
}
